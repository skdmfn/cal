...
with col2:
    if unit_category == "길이":
        ...
        # Conversion factors relative to 'm' (meter). 1 unit = X meters
        conversions = {
        ...
        # Conversion factors relative to 'kg' (kilogram). 1 unit = X kilograms
        conversions = {
        ...
        def convert_temperature(val, from_u, to_u):
            ...
            if from_u == "°C":
                ...
                celsius = val - 273.15
            else: # Should not be reached due to selectbox
                return val 
        ...
        # Conversion factors relative to 'Pa' (Pascal). 1 unit = X Pascals
        conversions = {
        ...
        # Conversion factors relative to 'J' (Joule). 1 unit = X Joules
        conversions = {
...
try:
    if from_unit == to_unit:
        ...
        # Convert 'value' from 'from_unit' to its equivalent in the common base unit (e.g., meters for length)
        value_in_base = value * conversions[from_unit]
        # Then, convert from this base unit value to the 'to_unit'
        result = value_in_base / conversions[to_unit]
            
    st.markdown(f"**변환 결과:** `{value:.4f} {from_unit}` **=** `{result:.4f} {to_unit}`")

except KeyError:
    st.error("단위 변환 오류: 선택된 단위 조합에 대한 변환 정보를 찾을 수 없습니다. (내부 오류)")
except Exception as e:
    st.error(f"단위 변환 중 예상치 못한 오류가 발생했습니다. 입력 값을 확인해주세요. 오류: {e}")
...
if st.button("➕ 새 자료 저장하기", key="save_note_button"):
    if title.strip() == "" or content.strip() == "":
        ...
        # Clear input fields after saving for better UX by resetting their values
        st.session_state.note_title = ""
        st.session_state.note_content = ""
        st.session_state.note_link = ""
        st.rerun() # Re-run the app to clear inputs and refresh the list
